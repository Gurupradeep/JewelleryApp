...
import com.example.gurupradeep.jewelery_app.Adapters.Loyality_Ornaments_adapter;
 ...
 */
public class Sceme_Ornaments extends Fragment {


    public Sceme_Ornaments() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v=inflater.inflate(R.layout.fragment_sceme__ornaments, container, false);
        r = (RecyclerView)v.findViewById(R.id.Schemes_Ornaments_rc);
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                Ornaments.add(cursor.getString(4));
            } while (cursor.moveToNext());
        }
        temp = new ArrayList<loyality_scheme>();
        for(int i=0;i<scheme_name.size();i++)
        {
            temp.add(new loyality_scheme(scheme_name.get(i),Ornaments.get(i)));
        }
        adapter = new Loyality_Ornaments_adapter(getContext(),temp);
        r.setAdapter(adapter);
        r.addOnItemTouchListener(new Loyality_Ornaments_adapter.RecyclerTouchListener(getActivity().getApplicationContext(), r, new Loyality_Ornaments_adapter.ClickListener() {
            @Override
            public void onClick(View view, int position) {
                mFragmentManager = getFragmentManager();
                mFragmentTransaction = mFragmentManager.beginTransaction();
                mFragmentTransaction.replace(R.id.loyality_ornnaments_layout, new Scheme1_details());
